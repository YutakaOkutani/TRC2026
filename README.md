...
* 構成のポイント
  ...
  * ログを解析するanalyzer.pyを用意した。使い方は自分のPCの "~/Downloads" に出力された "robust_log_*.csv" を置いて、自分のPCでanalyzer.pyを実行する。これにより、最新のログが自動で分類、グラフ化、可視化される。出力データは"~/analyzer/outputs/robust_log_*" にすべて置かれる。Plotlyを使っているので、.htmlを開くと、ウェブブラウザ上で細かくグラフを見られるようになっている。注意としては、ラズパイ上でこのコードを実行すると処理が非常に重くなる可能性があるので、実行はPC上のみにすることを推奨する。
