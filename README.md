...
* 構成のポイント
  ...
  * ログを解析するanalyzer.pyを用意した。使い方は自分のPCの~/Downloadsに出力されたRobust_log_*.csvを置いて、自分のPCでanalyzer.pyを実行する。これにより、最新のログが自動で解析され、ログのデータを分析、グラフ化、可視化される。Plotlyを使っているので、ウェブブラウザ上でインタラクティブにグラフを操作できるようになっている。
